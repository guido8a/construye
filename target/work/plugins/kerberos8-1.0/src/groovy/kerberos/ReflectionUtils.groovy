...
//		println "---kerberos: getAuditClass"
...
//		println "---kerberos: ${domainClassName} --> ${AuditLogEventClazz}"
		AuditLogEventClazz
	}

	static getTabla(domain) {
//		println "---kerberos domain: ${domain.toString()}"
//		println "---kerberos domain: ${domain.class}"
        def tabla = new GrailsDomainBinder().getMapping(Holders.grailsApplication.getDomainClass(domain.class.name)?.clazz)
//		println "---kerberos --> ${tabla.table.name}"
		tabla.table.name.trim()
